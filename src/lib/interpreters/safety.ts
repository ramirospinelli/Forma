 ...
 */
export const getMonotonyStatus = (monotony: number): RiskStatus => {
  if (monotony < 1.3)
    return {
      level: "optimal",
      label: "Saludable",
      color: "#4ECDC4",
      interpretation: "Variedad adecuada de estímulos de carga.",
    };
  if (monotony <= 1.8)
    return {
      level: "transition",
      label: "Variedad Baja",
      color: "#F4D35E",
      interpretation: "Poca variación en intensidad. Riesgo de estancamiento.",
    };
  if (monotony <= 2.2)
    return {
      level: "overload",
      label: "Monotonía Alta",
      color: "#FF9234",
      interpretation: "Demasiada carga similar. Riesgo de sobreuso.",
    };
  return {
    level: "danger",
    label: "Peligro: Monotonía",
    color: "#FF6B6B",
    interpretation: "Variedad nula. Riesgo inminente de sobreentrenamiento.",
  };
};
