 ...
 * Uses timeStream if provided to calculate exact durations between samples.
 */
export function calculateTimeInDynamicZones(
  ...
  if (!timeStream || timeStream.length !== heartRateStream.length) {
    // Fallback: Each sample counts as 1 second
