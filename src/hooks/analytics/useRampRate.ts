...
export const useRampRate = (data: LoadDataPoint[]) => {
  return useMemo(() => {
    ...
    // We calculate weekly delta for each day (Current CTL - CTL from 7 days ago if available, else start)
    for (let i = 1; i < data.length; i++) {
      const current = data[i];
      const prevIdx = Math.max(0, i - 7);
      const previous = data[prevIdx];
      ...
      let sum = 0;
      let count = 0;
      for (let j = Math.max(1, i - 27); j <= i; j++) {
        const d_curr = Number(data[j].ctl) || 0;
        const d_prev = Number(data[Math.max(0, j - 7)].ctl) || 0;
        sum += d_curr - d_prev;
        count++;
      }
